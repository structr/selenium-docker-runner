...
/**
 */
public class DoubleClickCommand extends AbstractSelectorAction {
	...
	public boolean execute(final Terminal out) {
		...
		if (elem != null) {
			...
			context.getActions().moveToElement(elem, offset.x, offset.y).doubleClick().perform();
		}
	...
	@Override
	public String usage() {
		return "doubleclick <selector> <x> <y> - creates a doubleclick event at the given element with the given optional x and y offsets.";
	}
}
